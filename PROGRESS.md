...
1. **เอกสารล็อตส่งมอบเขียนทับได้ ไม่มี hash** (จาก 8.2) — `lib/warehouse/upload-client.ts` ใช้ `upsert: true` + path ตายตัวต่อชนิดเอกสาร ซึ่ง **`44` §6.4 สั่งไว้อย่างนั้นจริง** ("1 ล็อต = 1 ไฟล์ต่อชนิด แนบใหม่ = ทับไฟล์เดิม") · ปัญหาคืออัปโหลดวิ่งจาก browser เข้า Supabase Storage **ตรง ๆ ไม่ผ่าน API ของเรา** ⇒ ไม่มีชั้นตรวจว่าล็อต `confirmed` ไปแล้ว และไม่เก็บ SHA-256 เลย · เลือก **(ก)** แก้ `44` §6.4 ให้ path ต่อเวอร์ชัน + เก็บ hash หรือ **(ข)** ย้ายอัปโหลดไปหลังบ้าน — ทั้งคู่กระทบ spec/design ⇒ ต้องมีมติ PO
2. **แจ้งเตือนของ job หายถาวรเมื่อ dispatch ล้ม** (จาก 8.2) — `lib/assignments/timeout-job.ts` / `lib/advances/overdue-job.ts` เรียก `dispatchNotificationAwaited()` **นอก** tx และไม่มี try/catch ⇒ dispatch ล้มหลัง commit = job ล้มทั้งที่สถานะเปลี่ยนแล้ว และรอบหน้าไม่หยิบซ้ำ (at-most-once) · แก้ให้ถูกต้องต้องมี **outbox table** ⇒ ต้องมี DEC ใหม่
3. **รายการเบิกที่ไม่มีอัตราภาษีทั้งสองระดับ ⇒ API พัง 500** (8.3 ด่าน 2) — Manual Claim (`lib/claims/queries.ts`) และเบิกที่พัก (`lib/field/expense-queries.ts`) สร้าง expense โดย**ไม่มี `compPlanId`** และ payee ที่ระบบสร้างให้อัตโนมัติยังไม่ผูก Tax Profile ⇒ `resolveWhtRate()` ไม่มีทั้ง Payee-level และ Plan-level → โยน `RangeError` (ไม่ใช่ `ModuleError`) → หลุดเป็น **500** ⇒ (ก) หน้าคิวอนุมัติค่าตอบแทน**ทั้งหน้า**พัง รายการคนอื่นหายหมด (ข) ถ้ารายการนั้น `approved` แล้ว → **สร้างรอบจ่ายเงินไม่ได้ทั้งรอบ** · แก้ต้องเลือกกติกาภาษี ห้ามเดาอัตราเงียบ ๆ: **(ก)** ใช้ 0% + warning ตัวใหม่ (ต้องเพิ่มใน `24` และรายชื่อ "เตือนไม่บล็อก" ที่ Rule 04 ล็อกไว้ = ต้องมีมติ PO) · **(ข)** คืน error code ใหม่จาก `24` แล้วบังคับให้ผูก Tax Profile ก่อนอนุมัติ · **(ค)** ตั้งค่าอัตราเริ่มต้นระดับองค์กรใน `13` — ทั้งสามทางกระทบภาษีจริง ⇒ ต้องนักบัญชี + PO
4. **ตั้งค่าเทมเพลตเอกสารภาษี (`13` §6.13) เป็น config ที่ไม่มีผล** (8.3 ด่าน 3) — `lib/settings/queries/tax-doc-templates.ts` ถูกอ่านที่ route ตั้งค่าเท่านั้น · `components/pdf/tax-invoice.tsx` / `wht-certificate.tsx` hardcode `<Page size="A4">` ขึ้นหัวสองภาษาเสมอ และไม่เคยอ่าน logo / `footer_note` / `signature_image_url` ⇒ บัญชีตั้งค่าแล้วกดพิมพ์ก็ไม่มีอะไรเปลี่ยน · ส่วน paper size / footer / ภาษา แก้ได้ตรง ๆ **แต่โลโก้กับลายเซ็นต้องตัดสินก่อน**ว่าโหลดรูปจาก Supabase Storage ตอน render ฝั่ง server อย่างไร (ไฟล์หาย/เน็ตล่ม = ออกใบกำกับภาษีไม่ได้) ⇒ ต้องมีมติเรื่องกลไกรูปภาพก่อนลงมือทั้งก้อน
