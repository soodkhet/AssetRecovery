...
import { CapabilityAccessLevel, FunctionalGroup, RoleGroup, WhtFilingForm } from '@/lib/generated/prisma/enums'
 ...
 * VAT → tax profiles → finance policy → capabilities
 *
 * ⚠️ Immutable (`02` §13): role ที่ `isSeed = true` ห้ามลบ/เปลี่ยน `name`/`roleGroup`
 * ⚠️ ผูก role ↔ capability (`role_capabilities`) **ไม่อยู่ในไฟล์นี้** — เป็นงาน Phase 1.6 (ไฟล์ 07/25)
 *    Superadmin ไม่มี record โดยนิยาม enforce ที่ middleware (DEC-009)
 */
 ...
 *
 * 🔒 = "✅ only" ตาม `25` §16.1 — มอบให้ role อื่นไม่ได้ (บังคับจริงตอน Phase 1.6)
 */
...
async function main() {
  ...
  const [roleCount, matrixCount, capabilityCount] = await Promise.all([
  ...
  console.log(
    `[seed] เสร็จ — organization 1 · roles ${roleCount} (ต้อง 15) · capabilities ${capabilityCount} ` +
      `(ใน Functional Matrix ${matrixCount} ต้อง 37) · VAT 7% · tax profiles 2 · finance policy 1`,
  )
