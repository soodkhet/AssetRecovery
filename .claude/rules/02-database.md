...
- **Prisma เป็นเจ้าของ schema แต่เพียงผู้เดียว** — ห้ามเปิด Supabase **Branching**, ห้ามใช้ GitHub migration sync ของ Supabase (โฟลเดอร์ `supabase/migrations` ต้องไม่มีใน repo), ห้ามสร้าง/แก้ตารางผ่าน Supabase Dashboard/SQL Editor — ทุกการเปลี่ยน schema ผ่าน `prisma migrate` เท่านั้น (Supabase GitHub integration ที่เชื่อมไว้ตอนสร้าง project เป็น inert — อย่าไปเปิดฟีเจอร์ให้มัน)
