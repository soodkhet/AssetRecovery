...
- **`lib/reports/executive/*-report.ts` (pure 3 ไฟล์ + เทสต์ 25 เคส)** — ตัวแปลงข้อมูลเป็น `columns`/`rows`/`kpis`/`totalRow` ของ E1–E3 ตาม `96` §6-E · ทั้งหมวดแคช **รายวัน** (`96` §8 — `runReport()` จัดการให้)
  - **E1 KPI ภาพรวม** — การ์ด 6 ตัวตามเอกสาร (รายได้ / กำไรขั้นต้น / Margin % / เคสทั้งหมด / % สำเร็จ / AR ค้างรับ) + badge เทียบช่วงก่อนหน้า · **ตาราง = เทรนด์ 12 เดือนย้อนหลัง** (เดือน/รายได้/ต้นทุน/กำไร/margin/เคส/สำเร็จ/ไม่สำเร็จ/%สำเร็จ) ซึ่งเป็นแหล่งข้อมูลของกราฟเส้นทั้งสองตัว · `lastMonthlyPeriods()` เดินเดือนถอยหลังผ่าน `resolveReportPeriod()` (ปฏิทินไทย) ⇒ ข้ามปี/เดือนสั้นถูกเสมอ
  - **E2 Scorecard รายบริษัท** — คอลัมน์ตรงเอกสาร (บริษัท/เคส/%สำเร็จ/รายได้/กำไร/Margin/AR ค้าง/เทรนด์) · เรียงตามรายได้ ⇒ ใช้เป็นแหล่งของกราฟ Top 5 บน E1 ได้ตรง ๆ
  - **E3 Scorecard รายทีม** — คอลัมน์ตรงเอกสาร (ทีม/ประเภท/พนักงาน/เคส/%สำเร็จ/TAT เฉลี่ย/ต้นทุน/กำไร) · TAT คิดจาก**เคสที่ปิดในช่วง** เท่านั้น ทีมที่ยังไม่มีเคสปิดได้ `null` ("—") ไม่ใช่ 0 วัน
- **ไม่มีสูตรใหม่แม้แต่บรรทัดเดียว** — กำไร/margin = `grossProfit()` (3.1 · `22` §6.12) · % สำเร็จ = `successPctOf()` ของ O1 (ต่อไปยัง `successRate()` ตัวกลาง `40` §6.2 — 6.5 เปลี่ยนจาก private เป็น export) · TAT = `elapsedHours()`/`hoursToDays()` ของ O2 · AR = `arOutstandingSatang()` (`22` §6.11)
- **`lib/reports/executive/providers.ts`** — ชั้น DB ของทั้ง 3 รายงาน ต่อเข้า `REPORT_PROVIDERS` · ยอดรายได้/ต้นทุนมาจาก `loadProfitEntries()` + `summarizeProfitability()` **ชุดเดียวกับ F1 และแท็บกำไรของไฟล์ 21** · ลูกหนี้มาจาก `loadArAgingCompanies()` ที่แยกออกมาจาก provider ของ F3 ในเฟสนี้ (F3 เรียกตัวเดียวกันต่อ) ⇒ ตัวเลข AR ของ E1/E2/F3 มาจาก query ชุดเดียวกัน
- **UI** — `<ReportLineChart>` (Recharts) เป็น shared ตัวใหม่ข้าง `<ReportBarChart>` · `<KpiSummaryScreen>` ลงทะเบียนที่ `<ReportScreen>` (E2/E3 เป็นตารางล้วน ⇒ ใช้ `<ReportView>` ตรง ๆ) · ช่วงเวลาเริ่มต้นของ E1 = **"ปีนี้"** เพื่อให้ตรงกับคำว่า YTD ในเอกสาร · เวลา refresh ล่าสุด/ปุ่ม "รีเฟรชตอนนี้"/ป้ายเตือนข้อมูลเก่ากว่า 24 ชม. เป็นของโครงกลาง 6.1 อยู่แล้ว (`96` §13 E1)
- **เทสต์ระดับ DB 11 เคส** (`executive-reports.db.test.ts`) เดินผ่าน `runReport()` ตัวจริง: KPI 6 ตัวตรงกับข้อมูลที่ seed, badge MoM เทียบเดือนก่อน, เทรนด์ 12 เดือนครบ (มีเดือนข้ามปี + เดือนว่างต้องมีแถว), เคสที่ยังไม่ปิดไม่เข้าตัวหาร %สำเร็จ, บิล `draft`/บิลที่ปิดยอดครบไม่นับเป็น AR, ยอดรวมของ E2 = KPI ของ E1 ในช่วงเดียวกัน, TAT รายทีม, **การเงินเรียก E1/E2/E3 = 403 ครบทั้ง 3 ตัว** (`96` §14)

### การตัดสินใจระหว่างทาง

- **KPI ของ E1 ใช้ช่วงที่ผู้ใช้เลือก แต่ตาราง/กราฟเป็น 12 เดือนย้อนหลังเสมอ** — เอกสารเขียน "YTD" (คงที่) ขณะที่ `96` §11 บังคับให้ทุกรายงานมี Date Range Picker · ทางออกคือให้ค่าเริ่มต้นเป็น "ปีนี้" (= YTD) แล้วเขียนความต่างของขอบเขตไว้ใน `note` ของรายงาน — ไม่ตัด Date Range Picker ทิ้งและไม่บิดกราฟตามช่วงจนเทรนด์ 12 เดือนหาย
- **กราฟ Top 5 บริษัทบน E1 อ่าน payload ของ E2** ผ่าน `useReportData('company-scorecard')` แทนการคำนวณอันดับซ้ำในฝั่ง E1 — สองรายงานอยู่หมวดเดียวกัน (สิทธิ์ชุดเดียวกัน แคชรายวันเหมือนกัน) ⇒ ตัวเลขบนกราฟกับหน้า E2 ตรงกันเสมอ และ payload ของ E1 ยังคงเป็น "ตารางเดียว" ที่ไฟล์ export ใช้ได้ตรง (`96` §13)
- **เทรนด์ 12 เดือนเรียก `loadProfitEntries()` เดือนละครั้ง** (แทนการเขียน query จัดกลุ่มรายเดือนชุดใหม่) — ยอมจ่ายจำนวน query เพื่อไม่ให้มีตรรกะ adjustment/ต้นทุนตรงชุดที่สองในระบบ · รายงานหมวด E แคชรายวันอยู่แล้วจึงไม่ใช่ต้นทุนต่อการเปิดหน้า
- **E2 ตัดบริษัทที่ไม่มีความเคลื่อนไหวและไม่มีหนี้ค้างออก** แต่ **บริษัทที่ "มีแต่หนี้ค้าง" ต้องยังอยู่ในตาราง** — หนี้ที่ค้างข้ามงวดคือสิ่งที่ผู้บริหารต้องเห็นที่สุด · E3 ตรงข้าม: แสดง**ทุกทีมที่ยังใช้งานอยู่** แม้ยอดเป็นศูนย์ เพราะ "ทีมที่ไม่มีงานเลย" คือข้อมูลบริหารในตัวมันเอง
- **AR ไม่มี badge MoM** — เป็นยอดคงค้าง ณ วันที่ ไม่ใช่ยอดของงวด การเทียบกับ "ช่วงก่อนหน้า" จึงไม่มีความหมาย

### จุดที่คนถัดไปควรรู้

- **รายงานเปิดใช้งานครบ 17/17 ตัวแล้ว** ⇒ `lib/reports/run.test.ts` เลิกใช้ "รายงานที่ยังไม่มี provider" ของจริง เปลี่ยนเป็น **ถอน provider ออกจากทะเบียนชั่วคราว** (`delete REPORT_PROVIDERS[id]` — `afterEach` คืนให้) · ยาม `REPORT_NOT_FOUND` ยังต้องอยู่สำหรับรายงานที่จะเพิ่มในอนาคต
- `successPctOf()` (นิยาม "% สำเร็จของเคสที่ปิดแล้ว") ถูก export จาก `lib/reports/operations/success-rate-report.ts` แล้ว — **รายงาน/หน้าจอใหม่ต้องเรียกตัวนี้ ห้ามเขียนสูตรหารเอง**
- `loadArAgingCompanies()` ย้ายมาเป็น export ของ `lib/reports/finance/providers.ts` — ใครต้องการยอดลูกหนี้ระดับบริษัทให้เรียกตัวนี้ + `arOutstandingSatang()` ห้าม query `billing_batches` ชุดใหม่
- ไม่มี migration ใหม่ในเฟสนี้ ⇒ ไม่ต้องรัน `pnpm db:deploy`

---

## Phase 6.4 — รายงานหมวด A (A1–A4)

**วันที่**: 2026-08-15 · **commit**: `ec43687` · **branch**: `auto/phase-6.4`

### สิ่งที่ทำ

- **`lib/reports/accounting/*-report.ts` (pure 4 ไฟล์ + `period-window.ts` + เทสต์ 29 เคส)** — ตัวแปลงข้อมูลเป็น `columns`/`rows`/`kpis`/`totalRow` ของ A1–A4 ตาม `96` §6-A · ทั้งหมวดเป็น **real-time ไม่แคช** (`96` §8)
  - **A1 สรุป WHT รายเดือน** — อ่าน `wht_filing_summaries` **ตรง ๆ** (`96` §7) เพราะ 4.5 คำนวณยอดไว้แล้วโดยตัดใบ 50 ทวิ ที่ `cancelled` ออก (`refreshFilingSummary()` ถูกเรียกทุกครั้งที่ออก/ยกเลิกใบ) — รายงานรวมยอดจากใบเองซ้ำอีกชั้น = ใบที่ยกเลิกกลับมาโผล่ · รวม WHT = ภ.ง.ด.3 + ภ.ง.ด.53 · "เลยกำหนดยื่น" เป็น**ข้อความประกอบของสถานะ `pending`** (`FILING_OVERDUE_WARNING` เตือนไม่บล็อก) ไม่ใช่สถานะใหม่ในสคีมา
  - **A2 สรุปใบกำกับภาษี** — มิติ `month|company` · **ใบที่ `cancelled` ไม่เข้ายอดและไม่นับใน "จำนวนใบ" แต่ต้องเห็นจำนวนที่ยกเลิก** (`31` §8 — ใบยกเลิกห้ามลบ ต้องตามรอยได้) · ยอดก่อน VAT/VAT/สุทธิ มาจาก **snapshot ของบันทึกขาย** ไม่คิด 7% ใหม่ (Rule 01) · กลุ่มรายเดือนคิดจาก `invoice_date` (ปฏิทินไทย) — ฟิลด์เดียวกับที่ใช้กรองช่วง จึงไม่มีเคส "กรองด้วยฟิลด์หนึ่ง จัดกลุ่มด้วยอีกฟิลด์"
  - **A3 ประวัติส่งออกชุดข้อมูลบัญชี** — แสดง **ทุกเวอร์ชัน** ของทุกงวด (`37` §6.2 export ห้ามทับ ต้องขึ้น version) · **งวดที่ยังไม่เคย export มีแถวของตัวเอง** สถานะ "ยังไม่ส่งออก" ค่าอื่นเป็นค่าว่าง · จำนวนไฟล์นับเฉพาะคีย์ `01`–`08` (หน้าปก/`.zip` ไม่ใช่ไฟล์ข้อมูล) · KPI นับเป็น**จำนวนงวด** ไม่ใช่จำนวนเวอร์ชัน
  - **A4 สรุปข้อยกเว้นรายงวด** — ตัวนับทั้งหมดมาจาก `summarizeExceptionCounts()` ของ 4.1 (ตัวเดียวกับหน้าบัญชี) · **`authorized` มีคอลัมน์แยกจาก `resolved` เสมอ** (`34` §6.3 มาตรการกันหายเงียบ) · KPI ตัวบล็อก = critical ที่ยัง `open` เท่านั้น · งวดที่ไม่มีข้อยกเว้นเลยยังมีแถวยอด 0
- **`period-window.ts`** — `accountingPeriodWindow()` แปลงช่วงวันที่ของ Date Range Picker เป็นช่วง `(year_be, month)` ด้วย `periodKeyOf()` (ปฏิทินไทย) · **งวดที่พาดผ่านแม้บางส่วนต้องอยู่ในช่วง** (custom 15/06–20/07 ⇒ ได้ทั้งสองงวด — ตัดทิ้ง = ทำข้อมูลหาย)
- **`lib/reports/accounting/providers.ts`** — ชั้น DB ทั้ง 4 รายงาน ลงทะเบียนเข้า `REPORT_PROVIDERS` · อ่านอย่างเดียว · กรองงวดแบบ lexicographic (`yearBe > start` OR `yearBe = start AND month >= start.month` … ) ไม่ไล่แจกแจงเดือน เพราะช่วงกำหนดเองกินหลายปีได้
- **UI** — `<WhtSummaryScreen>`/`<TaxInvoiceScreen>`/`<ExceptionSummaryScreen>` ลงทะเบียนที่ `<ReportScreen>` (A3 ใช้ `<ReportView>` ตรง ๆ) · ใช้ `<ReportBarChart stack>` ของ 6.3 โดยไม่แก้คอมโพเนนต์กลางแม้แต่บรรทัดเดียว
- **เทสต์ระดับ DB 12 เคส** (`accounting-reports.db.test.ts`) เดินผ่าน `runReport()` ตัวจริง: ยอด A1 ตรงกับสรุปที่บันทึกไว้, งวดนอกช่วงไม่หลุดเข้ามา, ป้ายเลยกำหนด, ใบยกเลิกไม่เข้ายอด, VAT จาก snapshot (ทดสอบด้วยอัตราสมมุติ 10% เพื่อพิสูจน์ว่าไม่ได้คิด 7% เอง), ทุกเวอร์ชันขึ้นครบ, งวดยังไม่ส่งออกมีแถว, `authorized` แยกจาก `resolved`, **การเงินเรียกหมวด A = 403 ครบทั้ง 4 ตัว**

### การตัดสินใจระหว่างทาง

- **A1 ไม่ re-aggregate จาก `wht_certificates`** แม้ §7 จะเขียนว่า join กับตารางใบ — ยอดที่ถูกต้องตาม `33` §9/§16 คือยอดในตารางสรุปซึ่งถูกคำนวณใหม่ทุกครั้งที่ใบเกิด/ถูกยกเลิกอยู่แล้ว การรวมซ้ำในรายงานมีแต่จะทำให้สองหน้าจอไม่ตรงกัน
- **A3 แสดงทุกเวอร์ชัน ไม่ใช่เวอร์ชันล่าสุดต่อรอบ** (mockup โชว์แถวละงวด) — ชื่อรายงานคือ "Export History" และ `37` ออกแบบ versioning มาเพื่อให้ตรวจย้อนได้ การซ่อนเวอร์ชันเก่าคือการซ่อนประวัติการส่งซ้ำ
- **A3 คอลัมน์ "เอกสารแนบ" เป็นค่าว่างทุกแถว** ไม่ใช่ 0 — เอกสารแนบยังไม่รวมในชุด export (`37` §7.1 · บันทึกไว้ตั้งแต่ 4.6) ⇒ 0 จะอ่านว่า "ตรวจแล้วไม่มี" ซึ่งไม่จริง
- **A4 เพิ่มคอลัมน์ "ผ่านแบบมีข้อยกเว้น" นอกเหนือจาก 6 คอลัมน์ใน §6-A4** — บังคับโดย `34` §6.3 (ห้ามนับปนกับ "แก้ไขแล้ว") ซึ่งอยู่เหนือกว่าในลำดับความสำคัญของเอกสาร · ไม่ใส่คอลัมน์ "สถานะรอบ" ของ mockup เพราะซ้ำกับข้อมูลที่คอลัมน์ "ยังเปิดอยู่" บอกอยู่แล้ว
- **หมวด A ไม่ใช้ `ctx.teamIds`** — ข้อมูลเป็นระดับองค์กร (งวดบัญชี/ภาษี) ไม่มีมิติทีม และผู้ที่มีสิทธิ์เห็น (บัญชี/ผู้บริหาร) เป็น scope ระดับองค์กรอยู่แล้ว

### จุดที่คนถัดไปควรรู้

- `lib/reports/run.test.ts` ใช้ **E1** เป็นตัวอย่าง "รายงานที่ยังไม่มี provider" แล้ว — **Phase 6.5 (หมวด E) ต้องเปลี่ยนเทสต์นี้** ไปใช้วิธีอื่น (ลงทะเบียน provider ปลอมแล้วลบ) เพราะจะไม่เหลือรายงานที่ยังไม่เปิดใช้งานอีก
- เทสต์ระดับ DB ของหมวด A ต้อง **ปิด trigger `trg_tax_invoices_no_delete` ชั่วคราวตอนล้างข้อมูล** (ใบกำกับภาษีลบไม่ได้ตาม `02` §13) — ทำแบบเดียวกับ `handover_lots` ของ 2.13/6.3
- ไม่มี migration ใหม่ในเฟสนี้ ⇒ ไม่ต้องรัน `pnpm db:deploy`
