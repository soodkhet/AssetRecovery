...
- **[default]**: ตัดสองแถวนี้ออกจาก `90` §6.3 (ไม่ใช่ requirement จริง) · ทางเลือก: เพิ่ม `payout_batch_status = 'failed'` + `exceptions.due_date` ลง `02` แล้วค่อยต่อสาย · **บล็อก**: ไม่บล็อกงานใด · **คำตอบ**: ✅ **ใช้ default ตามมติ PO 15/08/2569** — ถอน `payout_batch.failed` และ "Exception ใกล้ deadline" ออกจาก `90` §6.3 (v2.3 พร้อม changelog อธิบายเหตุผลและเงื่อนไขที่จะกลับมาต่อสายได้) · โค้ดไม่ต้องแก้ (ไม่เคยมีจุด emit อยู่แล้ว) — อัปเดตคอมเมนต์ `NOTIFICATION_ONLY_EVENTS` ใน `lib/notifications/events.ts` ให้ตรงมติ · สองชื่อนี้ยังอยู่ในแค็ตตาล็อกฝั่งแสดงผลเพื่อให้แถวเก่า/ข้อมูลนำเข้าตกกลุ่มสีถูก
...
- **[default]**: เพิ่ม audience `accounting`/`finance` ให้เมนู `settings.audit-logs` + `settings.jobs` (แก้ `06` §7.2 คู่กัน) เพราะสองหน้านี้อ่านอย่างเดียวและ API คุมสิทธิ์อยู่แล้ว · ทางเลือก: ถอน grant ออกจาก default matrix ให้ตรง `06` · **บล็อก**: ไม่บล็อกงานใด (ฟีเจอร์ใช้ได้ครบสำหรับ Superadmin/บริหาร) · **คำตอบ**: ✅ **ใช้ default ตามมติ PO 15/08/2569** — `06` §7.2 v2.2: เมนู "การตั้งค่า" = 🔸 (เห็นบางส่วน) สำหรับการเงิน/บัญชี เห็นเฉพาะ **บันทึกการใช้งาน** + **งานเบื้องหลัง** · แท็บที่ตั้งค่าจริงทุกตัวยังเป็นของ Superadmin+บริหาร · `/settings` เปลี่ยนเส้นทางไป **แท็บแรกที่ผู้ใช้เห็น** (`firstVisibleChildPath()`) แทนแท็บตายตัว ไม่งั้นสอง role นี้จะถูกเด้งกลับแดชบอร์ดตั้งแต่หน้าแรก · **สั่งงาน/retry ยังทำไม่ได้** — บังคับที่ API (`manage` + capability ของงานปลายทาง / Superadmin) เมนูเป็นชั้น UX เท่านั้น (DEC-002)

### ✅ D18 — `96` §6-O2/O4 อ้าง `slaAlertHours` ที่ไม่มีอยู่จริงทั้งใน `03` และ `02` (พบตอนเริ่ม Phase 6.3)
- **ปัญหา**: `96` §6-O2 เขียนว่า "SLA threshold ดึงจาก `slaAlertHours` ใน Finance Settings (ไฟล์ 03)" และ §6-O4 ใช้เงื่อนไข `created_at + slaAlertHours < now` แต่ (1) ไฟล์ `03` **ไม่เคยนิยามค่านี้** — บรรทัดที่ 21 เขียนตรงข้ามว่า "ตัวเลข SLA ที่ยังไม่ตกลง (ดู Open Items ข้อ 18)" (2) `02` ไม่มีคอลัมน์นี้ในตารางตั้งค่าใดเลย (`finance_policy_settings` ไม่มี · `assignment_policy_settings` มีแค่ `reassign_timeout_hours`/`accept_deadline_hours` ซึ่งคนละความหมาย) (3) ตัวเลขเกณฑ์เป็นนโยบายธุรกิจที่ยังค้างอยู่ที่ `DECISIONS-NEEDED.md` §3.2 ⇒ **O2/O4 คำนวณไม่ได้เลยถ้าไม่มีค่านี้**
- **[default]**: เพิ่ม `assignment_policy_settings.sla_alert_hours` ระดับองค์กร + แท็บตั้งค่า ค่าเริ่มต้น 72 ชม. · **บล็อก**: บล็อก 6.3 (O2/O4) · **คำตอบ**: ✅ **ใช้ default ตามมติ PO 15/08/2569** — implement ใน Phase 6.3: คอลัมน์ใหม่ `sla_alert_hours INTEGER NOT NULL DEFAULT 72 CHECK (> 0)` (`02` v4.4 · migration `20260815210000_assignment_policy_sla_alert_hours`) + แท็บตั้งค่าที่ `13` §6.14 (แท็บที่ 14 ของหน้าตั้งค่าบัญชี/การเงิน · บังคับ `reason`) + `96` v2.2 แก้ที่มาของค่าให้ตรงความจริง · **ขอบเขต: ใช้กับรายงาน O2/O4 เท่านั้น** — ไม่บล็อก flow ใด ไม่ auto-reassign ไม่มีค่าปรับ (คำถาม "เกิดอะไรขึ้นเมื่อ breach" ของ `DECISIONS-NEEDED.md` §3.2 ยังค้างต่อไป และไม่บล็อกงานใด)
