...
| v4.4 | 15/08/2569 | **มติ PO 15/08/2569 ตอบ `[[NEEDS_DECISION]]` ตอนเริ่ม Phase 6.3 (D18)**: เพิ่ม `assignment_policy_settings.sla_alert_hours INTEGER NOT NULL DEFAULT 72 CHECK (> 0)` — ไฟล์ `96` §6-O2/O4 อ้าง `slaAlertHours` ว่าอยู่ "ใน Finance Settings (ไฟล์ 03)" แต่ไฟล์ `03` ไม่เคยนิยามค่านี้ (§18 ระบุเองว่าตัวเลข SLA ยังไม่ตกลง) และไฟล์นี้ไม่มีคอลัมน์รองรับ ⇒ PO เคาะให้เก็บ **ระดับองค์กร** ในตารางค่าตั้งของงานมอบหมาย (ไฟล์ 40 §6.4) ค่าเริ่มต้น **72 ชั่วโมง = 3 วัน** · ใช้เฉพาะ **รายงาน O2/O4 (อ่านอย่างเดียว)** — ไม่บล็อก flow ใด ไม่ auto-reassign ไม่มีค่าปรับ · หน้าจอตั้งค่าอยู่ที่ไฟล์ `13` §6.14 · **ไม่มีการลบ/เปลี่ยนคอลัมน์เดิม ไม่มีตาราง/enum ใหม่** — enum คงที่ **61 ตัว** · คงที่ **60 tables** · migration: `20260815210000_assignment_policy_sla_alert_hours` |
...
-- แก้ 14/08/2569 v4.2 (Phase 2.8 — sync กับไฟล์ 41 §10 ซึ่งเป็นเจ้าของ state machine ของ assignment)
CREATE TYPE assignment_status AS ENUM (
  'pending_accept',       -- มอบหมายแล้ว รอพนักงานกดรับ
  'accepted_unscheduled', -- รับงานแล้ว ยังไม่จัดวันที่
  'scheduled',            -- จัดวันที่แล้ว กำลังดำเนินงาน
  'closed_success',       -- ปิดงานสำเร็จ (terminal ยกเว้นถูกตีกลับ)
  'closed_fail',          -- ปิดงานไม่สำเร็จ (terminal ยกเว้นถูกตีกลับ/recycle)
  'needs_revision',       -- ถูกเจ้าหน้าที่อนุมัติเคสตีกลับหลักฐาน (`41` §10.1) รอแก้แล้ว resubmit
  'reassigned_away'       -- ถูกโอนไปพนักงานคนใหม่ — terminal สำหรับคนเดิม ไม่นับ success_rate
);
...
-- เพิ่ม 14/08/2569 v4.2 (Phase 2.8 — ไฟล์ 41 §6.4.1)
CREATE TYPE travel_origin_source AS ENUM (
  'gps_auto',        -- ค่าเริ่มต้นจาก device GPS ตอนกดปุ่ม "เริ่มงาน"
  'manual_adjusted'  -- พนักงานลากปรับตำแหน่งบนแผนที่เอง
);
